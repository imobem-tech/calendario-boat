...
(async () => {
  try {
    ...
    let encontradas = 0;
    for (const t of ofxDatasOvl.slice(0, 30)) {
      ...
      if (r.rows.length > 0) {
        encontradas++;
        console.log(`✅ MATCH #${encontradas}: ${t.data} | R$ ${t.valor.toFixed(2)}`);
        console.log(`   OFX: ${t.descricao}`);
        console.log(`   BD:  ${r.rows[0].descricao_original}\n`);
      }
    }

    console.log(`\n📊 RESULTADO: ${encontradas} duplicatas encontradas de ${Math.min(30, ofxDatasOvl.length)} testadas\n`);
    await pool.end();
